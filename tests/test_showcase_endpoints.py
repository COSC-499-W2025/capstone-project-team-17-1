...
def _seed_snapshot(db_path: str, project_id: str, snapshot: dict):
...
def _seed_contributor(db_path: str, project_id: str, contributor: str = "alice"):
    conn = sqlite3.connect(db_path)
    try:
        conn.execute(
            """
...
def _seed_portfolio_showcase_variant(db_path: str, project_id: str, summary: str, source: str = "custom") -> dict:
    conn = sqlite3.connect(db_path)
    try:
...
def test_showcase_users_returns_list(showcase_client):
    ...
    _seed_snapshot(db_path, "demo", {"summary": "seed"})
    _seed_contributor(db_path, "demo", "alice")
...
def test_showcase_get_portfolio_returns_saved(showcase_client):
    client, db_path = showcase_client
    expected = _seed_portfolio_showcase_variant(db_path, "demo1", "Showcase summary saved")
...
def test_showcase_query_alias_works(showcase_client):
    client, db_path = showcase_client
    expected = _seed_portfolio_showcase_variant(db_path, "demo3", "Alias summary")
...
def test_showcase_generate_creates_items_for_projects_with_snapshots(showcase_client):
    ...
    _seed_snapshot(db_path, "demo4", {"summary": "one"})
    _seed_snapshot(db_path, "demo5", {"summary": "two"})
...
def test_showcase_edit_requires_summary(showcase_client):
    client, db_path = showcase_client
    _seed_snapshot(db_path, "demo7", {"summary": "seed"})
...
def test_showcase_edit_persists(showcase_client):
    client, db_path = showcase_client
    _seed_snapshot(db_path, "demo8", {"summary": "seed"})
...
def test_showcase_auth_enforced_with_real_db(tmp_path):
    db_dir = str(tmp_path)
