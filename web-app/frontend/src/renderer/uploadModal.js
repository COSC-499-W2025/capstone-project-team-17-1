...
pullBtn?.addEventListener("click", async (e) => {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/github/pull?project_id=${encodeURIComponent(projectId)}`, {
      method: "POST"
    });
...
async function submitZipUpload() {
  ...
  const url = `${API_BASE}/projects/upload?project_id=${encodeURIComponent(projectId)}`;
...
export function renderGithubLogin(container) {
  ...
  const res = await fetch(`${API_BASE}/github/login?token=${encodeURIComponent(token)}`, {
    method: "POST"
  });
      ...
      uploadBtn?.addEventListener("click", async () => {
        ...
        try {
          const res = await fetch(
            `http://${API_BASE}/github/branches?owner=${owner}&repo=${name}`
          );
