...
function setModeUI(isAuthenticated, user) {
  ...
  if (modeBadge) {
    modeBadge.textContent = "Private Mode";
    modeBadge.classList.add("private");
    modeBadge.classList.remove("public");
  }
  ...
  if (customizationTab) customizationTab.classList.toggle("hidden", !isAuthenticated);
...
function renderSettingsProfile() {
  ...
  if (!currentUser) {
    el.innerHTML = "<h3>User Profile</h3><p>Please log in to view and edit your profile.</p>";
    return;
  }
...
export async function initAuthFlow() {
  ...
  if (!loginBtn || !logoutBtn || !submit || !toggleBtn || !modal || !usernameInput || !passwordInput || !error) {
    initNavigation();
    return;
  }
  ...
  initNavigation({
    onBeforeNavigate: async ({ tabKey, target }) => {
      ...
      const protectedTabs = new Set(["projects", "customization", "settings"]);

      if (protectedTabs.has(tabKey)) {
  ...
  if (user) {
    ...
    goToPage("projects", "projects-page");
  } else {
    setAuthToken(null);
    setModeUI(false, null);
    clearProtectedUI();
